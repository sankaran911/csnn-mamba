...
def show_gt(data, labels, class_nums, save_path, class_names=None, only_color=False):
    ...
    if class_names is None:
        class_names = [f"Class {i+1}" for i in range(class_nums)]
    ...
    for i in range(number_of_rows):
        for j in range(number_of_columns):
            rgb_hyperspectral_image[i,j,0] = pixels_normalized[cont,14]
            rgb_hyperspectral_image[i,j,1] = pixels_normalized[cont,7]
            rgb_hyperspectral_image[i,j,2] = pixels_normalized[cont,0]
            gt_thematic_map[i, j, :] = colors[gt[cont, 0]]
            cont += 1
    ...
    plt.imshow(gt_thematic_map)  # 标签图
    patches = [
    mpatches.Patch(color=colors[i + 1], label=str(class_names[i]))
    for i in range(min(class_nums, len(class_names)))
]
...
def show_pred(pred_test, labels, class_nums, dataset_name, save_path, removeZeroLabels=True):
    ...
    fig.savefig(save_path + '/maps_withZeros_' + dataset_name + '.png', dpi=300)
    ...
    if removeZeroLabels == True:  # 显示不带背景的预测结果图
        ...
        def show_gt(labels, class_nums, dataset_name, save_path):
            gt_map = np.zeros((labels.shape[0], labels.shape[1], 3))
            ...
            for i in range(labels.shape[0]):
                for j in range(labels.shape[1]):
                    gt_map[i, j, :] = colors[int(labels[i, j])]

            plt.figure(figsize=(8, 8))
            plt.imshow(gt_map)
            plt.axis("off")
            plt.savefig(
                save_path + "/" + dataset_name + "_GroundTruth.png",
                dpi=300,
                bbox_inches="tight",
                pad_inches=0,
            )
